...
def add_interval(head, interval):
    lower_intersect = upper_intersect = None

    node = head
    while node:
        if node.lower < interval.lower <= node.upper:
            lower_intersect = node
        if node.lower <= interval.upper < node.upper:
            upper_intersect = node

        if lower_intersect and upper_intersect:
            break
        node = node.next

    if lower_intersect and upper_intersect and lower_intersect == upper_intersect:
